...
import { eq as eq$1, or as or$1, ilike as ilike$1 } from "drizzle-orm";
import { z } from "zod";
import fs from "fs";
...
const ExtraConfigColumns = /* @__PURE__ */ Symbol.for(
...
const ExtraConfigBuilder = /* @__PURE__ */ Symbol.for(
...
const InlineForeignKeys = /* @__PURE__ */ Symbol.for(
  ...
  all(placeholderValues = {}) {
    return tracer.startActiveSpan("drizzle.execute", () => {
      ...
      return tracer.startActiveSpan("drizzle.driver.execute", (span) => {
        ...
        return this.client
          .query(this.rawQueryConfig, params)
...
const insertBookSchema = createInsertSchema(books).omit({
  id: true,
  createdAt: true,
});
...
class DatabaseStorage {
  async getBooks(params) {
    ...
    if (params?.status) {
      query.where(eq$1(books.status, params.status));
    }
    if (params?.category) {
      query.where(eq$1(books.category, params.category));
    }
    if (params?.search) {
      const searchPattern = `%${params.search}%`;
      query.where(
        or$1(
          ilike$1(books.title, searchPattern),
          ilike$1(books.author, searchPattern),
        ),
      );
    }
    return await query;
  ...
  async getBook(id) {
    if (!db) throw new Error("Database not initialized");
    const [book] = await db.select().from(books).where(eq$1(books.id, id));
    return book;
  }
  ...
  async updateBook(id, updates) {
    ...
    const [updated] = await db
      .update(books)
      .set(updates)
      .where(eq$1(books.id, id))
      .returning();
    return updated;
  }
  async deleteBook(id) {
    if (!db) throw new Error("Database not initialized");
    await db.delete(books).where(eq$1(books.id, id));
  }
}
class MemStorage {
  constructor() {
  ...
  async getBooks(params) {
    ...
    if (params?.status) {
      books2 = books2.filter((b) => b.status === params.status);
    }
    if (params?.category) {
      books2 = books2.filter((b) => b.category === params.category);
    }
    if (params?.search) {
...
const storage = db ? new DatabaseStorage() : new MemStorage();
const errorSchemas = {
  validation: z.object({
    message: z.string(),
...
async function registerRoutes(httpServer2, app2) {
  app2.get(api.books.list.path, async (req, res) => {
    try {
      const search = req.query.search;
      const status = req.query.status;
      const category = req.query.category;
      const books2 = await storage.getBooks({ search, status, category });
      res.json(books2);
    } catch (error) {
      res.status(500).json({ message: "Internal server error" });
    }
  });
  app2.get(api.books.get.path, async (req, res) => {
  ...
  app2.post(api.books.create.path, async (req, res) => {
    try {
  ...
  app2.put(api.books.update.path, async (req, res) => {
    try {
  ...
  app2.delete(api.books.delete.path, async (req, res) => {
...
async function startServer(port) {
  ...
  if (process.env.NODE_ENV === "production") {
    ...
    const { setupVite } = await import("./vite-CZAHI6po.js");
...
async function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    width: 1200,
    height: 800,
    icon: path.join(APP_ROOT, "public/Booklogo.ico"),
    webPreferences: {
